            ...
            try {
                switch($route.current.$$route.controller) {
                    ...
                    case 'NewsControl':
                        ...
                        break;
                    case 'SponsorsControl':
                        angular.element(document.querySelector('.sponsors-link')).addClass('active');
                        break;
                };
            }catch(e){
...
devfest.controller('NewsControl', function($scope, $http, Language){
    $scope.Language = Language;
    $http.
        jsonp('https://www.googleapis.com/plus/v1/activities?query=%23devfestlima&callback=JSON_CALLBACK&key=AIzaSyDssVSRzwj0vX4K3XsCVKJ-2LxCzXeiaNw').
...
devfest.controller('SponsorsControl', function($scope, $http, Language){
    $scope.Language = Language;
    $http.
        get('json/sponsors.json').
        success(function(response){
            $scope.sponsors = $scope.splitSponsors(response)
        });

    $scope.splitSponsors = function(list){
        var tmp = {
            0 : new Array(),
            1 : new Array(),
            2 : new Array()
